...
flowchart TB
    ...
    MAC["MAC / 链路传输<br/>CRC/FCS、PHY 选择<br/>不识别训练帧字段"]
    PHY["选定 PHY 与相邻节点"]
...
- 一次写入 TX FIFO 的宽写口；
- 保存本地已发送负载，供应答逐位比较；
- 接收 MAC/FIFO 回传记录，根据字 0、保留位和固定图样识别训练帧；
- 将字段识别结果与 MAC 提供的 CRC 状态共同作为接收门限；
- 在请求端要求应答与本次发送负载 128-bit 完全一致；
- 在应答端锁存请求并按 FSM 命令原样写回；
- 通过校验后锁存 `rx_fifo_timestamp` 并上送 FSM。

LINK 不生成或计算链路 CRC；MAC 不解析 LLTSM 字段，仅生成/校验 CRC/FCS 并
提供 `rx_fifo_crc_ok`。
...
| 方向 | 主要信号 |
|---|---|
| TOP -> FSM | `train_enable`, `train_start`, `train_abort`, `time_now` |
| FSM -> TOP | `train_start_ready`, `train_busy`, `train_done`, `result_*` |
| FSM -> LINK | `link_clear`, `link_tx_req_valid`, `link_tx_rsp_valid`, `link_expect_rsp`, `link_training_sequence` |
| LINK -> FSM | 对应 `ready`、`rx_req_valid`、`rx_rsp_valid`、锁存时间戳 |
