...
进入训练分支、何时中止以及何时根据 `branch_done` 返回业务状态。
...
TOP 在 `branch_enable=1` 期间必须保持节点、链路、通道和训练轮次配置稳定，并
...
- `link_tx_request_valid/ready`：发送新的固定训练帧；
- `link_tx_echo_valid/ready`：发送 LINK 已锁存的远端训练帧，所有位保持不变；
- `link_expect_response`：当前接收帧按本地请求的预期应答进行逐位比较；
- `link_training_sequence`：每次重复测量的序号；
- `link_clear`：TOP 中止或退出 LLTSM 分支时清除 LINK 上下文。

`lltsm_link` 向 FSM 提供：

- `rx_request_valid`：收到结构、目标和 CRC 均正确的远端请求；
- `rx_response_valid`：收到与本地已发送训练负载逐位一致的应答；
- `rx_response_timestamp`：通过校验后锁存的 MAC 接收时间戳；
- `rx_rejected`：收到 CRC、帧类别或负载不符合要求的记录，仅用于诊断。
...
远端应答直接使用收到并锁存的 128-bit 数据，因此特殊帧类别不变、协议标识不变、
所有数据位不变。请求端除检查 MAC 的帧类别与 CRC 结果外，还会与本次发送负载
进行 128-bit 精确比较，可拒绝旧轮次、错误序号或内容损坏的应答。

## 7. CRC 与字头职责

LLTSM 不产生也不校验链路 CRC/FCS。MAC 负责：

- 添加并识别训练帧使用的链路特殊字头或帧类别；
- Ethernet、RS-485 或自定义链路的地址/类型/长度和填充；
- CRC/FCS 生成与校验；
- 发送 PHY/通道选择；
- 接收时间戳参考点的实现。
...
只有远端应答路径固定且已标定时，`RESPONSE_COMPENSATION_CYCLES` 才能把结果解释
